...
N_CarPoleNL = size(mCarPoleNL,2);
N_ep = size(ep,2);
N_Mu = size(rhoMu,2);
N_Sigma = size(rhoSigma,2);

total_iterations = N_CarPoleNL*N_ep*N_Mu*N_Sigma;

CarPoleNL_ResultsPath = cell(total_iterations,1);
...
for i1=1:N_CarPoleNL
    for i2=1:N_ep
        for i3=1:N_Mu
            for i4=1:N_Sigma
                
                timeIterarionKey = tic;
                index = RemainingIterations(4,[[i1;i2;i3;i4],[N_CarPoleNL;N_ep;N_Mu;N_Sigma]],1,[]);
                ...
                StructKernelAmbiguity.m = mCarPoleNL(i1);
                ...
                CarPoleNL_ResultsPath{index} = CarPoleNL(N,CarPolepartitionNL,mCarPoleNL(i1),{StructNoAmbiguity,StructKernelAmbiguity},OuterLoopInfo);
...
PathsString = sprintf('./Results/results_%s/CarPoleNL/paths_%s.mat',char(java.net.InetAddress.getLocalHost.getHostName),CarPoleNL_ResultsPath{end}.FileName);
