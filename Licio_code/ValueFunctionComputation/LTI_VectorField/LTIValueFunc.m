classdef LTIValueFunc < ComputeValueFunction
    ...
    methods
        function obj = LTIValueFunc(number_of_points,time_horizon,type_vector_field,param)
            
            if ~strcmp(type_vector_field,'LTI')
                error('Type of vector field must be LTI')
            end

            obj = obj@ComputeValueFunction(prod(number_of_points),time_horizon);
        ...
        function obj = get_index_safety(obj,state_partition)
            ...
            if ~strcmp(state_partition.type_vector_field,'LTI') 
                error('This function can only be used for LTI vector field. Please check if this is your case.')
            end
