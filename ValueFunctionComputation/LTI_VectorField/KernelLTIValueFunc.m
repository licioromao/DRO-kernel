classdef KernelLTIValueFunc < LTIValueFunc
    ...
    methods
        function obj = KernelLTIValueFunc(number_of_points,time_horizon,...
            ...
            if strcmp(obj.type_value_func_computation,'KME')
                ...
                temp_x = linspace(obj.param.safe_set(1,1),obj.param.safe_set(1,2),number_of_points(1))';
                temp_y = linspace(obj.param.safe_set(2,1),obj.param.safe_set(2,2),number_of_points(2))';

                obj.points_to_estimate_RHKS_norm = [reshape(repmat(temp_x,1,number_of_points(2))',[],1),...
                                                                repmat(temp_y,number_of_points(1),1)];
...
%             end          
            ...
            if strcmp(obj.type_value_func_computation,'KME')
               LTI_kernel_obj = KernelBasedAmbiguity(objective_cost,obj.radius_ball,...
            ...
            LTI_kernel_obj.param = obj.param;
            LTI_kernel_obj.number_of_samples_KME = obj.param.number_of_points_KME;

            LTI_kernel_obj.current_state = current_state;
            LTI_kernel_obj.current_input = current_input; 
            
            switch obj.type_value_func_computation
                case 'Conservative'
                    LTI_kernel_obj = LTI_kernel_obj.solve_optimisation_conservative(state_partition);
                    out = LTI_kernel_obj.results_optimisation.opt_obj;
                case 'Matrix'
                    LTI_kernel_obj = LTI_kernel_obj.solve_optimisation(state_partition);
                    out = LTI_kernel_obj.results_optimisation.opt_obj;
                case 'QP'
                    LTI_kernel_obj = LTI_kernel_obj.solve_optimisation_QP(state_partition);
                    out = LTI_kernel_obj.results_optimisation.opt_obj;
                case 'KME'
                    LTI_kernel_obj = LTI_kernel_obj.solve_optimisation_KME(state_partition,obj.data_KME);
                    out = LTI_kernel_obj.results_optimisation.opt_obj;
                otherwise
                    not_implemented();
            end
            %[current_state,current_input*ones(2,1),out/1e4*ones(2,1)]
        ...
        function obj = backward_iteration(obj,state_partition,input_partition,...
            ...
            value_function(:,time_horizon+1) = initialise_value_func(grid_x,obj.param.Q); % initializing the value function
            ...
            print_inner_loop(total_iterations,0,0,obj.ambiguity_type,obj.param.outer_loop_info);
            ...
            for i = time_horizon:-1:1
                ...
                parfor j = 1:number_of_points % iterates over the number of points
...
%                     value_func_temp_input
%                     [aa,bb] = min(value_func_temp_input);
%                     aa
%                     current_state
%                     input_partition(bb)
                    [value_func_temp(j),opt_input_temp(j)] = min(-value_func_temp_input);
                end
                ...
                print_inner_loop(total_iterations,iterates,final_time,obj.ambiguity_type,obj.param.outer_loop_info);
            end
